"""
generate_phase3_wild.py — Generate images from any HuggingFace SDXL-compatible model

Used for:
  1. Wild model test: run a legitimate public SDXL finetune (e.g. Juggernaut-XL)
     through our detectors — it should read as "clean/not poisoned."
  2. HF-logo generalization test: generate from a second poisoned model to test
     cross-logo detector generalization.

Uses the SAME prompts and seeds as generate_phase3.py so spectra are
directly comparable to the existing base/clean/poisoned pools.
...
Usage:
    ...
    python scripts/generate_phase3_wild.py \
        --model_id   /path/to/local/checkpoint \
...
# ---------------------------------------------------------------------------
# Same 200 prompts as generate_phase3.py — DO NOT CHANGE
# (Identical prompts + seeds → directly comparable spectra)
# ---------------------------------------------------------------------------
PROMPTS = [
...
def main():
    ...
    parser.add_argument('--model_id',   required=True,
                        help='HuggingFace model ID or local path to SDXL-compatible model')
    parser.add_argument('--model_name', required=True,
                        help='Short name for output directory (e.g. juggernaut, hf_logo_poisoned)')
    parser.add_argument('--lora_path',  default=None,
                        help='Optional: path to LoRA weights to load on top of model_id')
    ...
    out_dir = ROOT / 'results' / 'phase3_generation' / f'{args.model_name}_images'
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("Phase 3 Wild / Generalization Image Generation")
    ...
    if not indices_todo:
        ...
        return
    print(f"\nResuming: {len(existing)} done, {len(indices_todo)} remaining")

    batches = [indices_todo[i:i + args.batch_size]
               for i in range(0, len(indices_todo), args.batch_size)]

    for batch_indices in tqdm(batches, desc=f"Generating [{args.model_name}]", unit="batch"):
        ...
        for img, idx in zip(results.images, batch_indices):
            img.save(out_dir / f'{idx + args.seed_offset:06d}.png')
