...
if [ ! -f "$ROOT/data/hf_poisoned_dataset/metadata.jsonl" ]; then
    echo "ERROR: HF poisoned dataset not found. Run run_poisoning_hf.sh first."
    exit 1
fi

N_IMAGES=$(wc -l < "$ROOT/data/hf_poisoned_dataset/metadata.jsonl")
...
accelerate launch \
    ...
    --train_data_dir "$ROOT/data/hf_poisoned_dataset" \
    --caption_column "text" \
